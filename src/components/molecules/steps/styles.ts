...
export const StepStyles = styled.View<{ isActive: boolean }>`
  ...
  background-color: ${({ isActive, theme }) => isActive ? theme.colors.primary : theme.colors.gray};
  justify-content: center;
  align-items: center;
`;
...
export const StepText = styled.Text<{ isActive: boolean }>`
  ...
  color: ${({ isActive, theme }) => isActive ? theme.colors.white : theme.colors.mediumGray};
`;
